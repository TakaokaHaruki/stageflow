...
export default function UserNameEditor({ user, onSaved }) {
  ...
  return (
    <>
      <button
        onClick={handleOpen}
        className="flex h-7 w-7 items-center justify-center rounded text-muted-foreground hover:text-primary transition-colors focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-ring select-none sm:h-5 sm:w-5"
        title="ユーザー名を編集"
        aria-label="ユーザー名を編集"
      >
        ...
        >
          ...
          >
            <div className="flex items-center justify-between mb-3">
              ...
              <button onClick={() => setOpen(false)} className="flex h-8 w-8 items-center justify-center rounded hover:bg-muted text-muted-foreground" aria-label="閉じる">
                <X className="w-4 h-4" />
              </button>
            </div>
