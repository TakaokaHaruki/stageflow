    ...
    <div className="max-w-xl">
      <div className="mb-3">
        ...
        <p className="mt-0.5 text-sm text-muted-foreground">
          チーフ権限以下のユーザーが閲覧できるイベントを最新{EVENT_LIMIT_COUNT}件に制限します。
        </p>
      </div>
      ...
      <div className="rounded-xl border border-border bg-card p-4">
        <div className="flex items-center justify-between gap-3">
          <div className="flex items-center gap-2">
            <Eye className="h-4 w-4 text-primary" />
            <span className="text-sm font-semibold">最新{EVENT_LIMIT_COUNT}件のみ閲覧可能</span>
          </div>
          {isLoading ? (
        ...
        {enabled && (
          <div className="mt-3 flex items-start gap-2 rounded-lg border border-amber-200 bg-amber-50 px-3 py-2 text-xs text-amber-800 dark:border-amber-800 dark:bg-amber-900/20 dark:text-amber-300">
            ...
            <span>管理者以外のすべてのユーザーは、最新{EVENT_LIMIT_COUNT}件のイベントのみ閲覧できます（管理者は全件閲覧可）</span>
          </div>
        )}
      </div>
    </div>
