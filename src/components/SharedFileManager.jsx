...
function VisibilityPanel({ file, roles, staffNames, onSave, onClose }) {
  ...
  const toggleStaff = (name) =>
    setAllowedStaffNames((prev) => prev.includes(name) ? prev.filter((x) => x !== name) : [...prev, name]);
    ...
    >
      ...
      {visibility === "roles" && (
        <div className="space-y-1 max-h-32 overflow-y-auto">
          {roles.map((r) => (
            <label key={r} className="flex items-center gap-2 cursor-pointer min-h-[32px]">
              <input
                type="checkbox"
      ...
      {visibility === "staff_names" && (
        <div className="space-y-1 max-h-32 overflow-y-auto">
          {staffNames.map((name) => (
            <label key={name} className="flex items-center gap-2 cursor-pointer min-h-[32px]">
              <input
                type="checkbox"
                className="rounded border-border"
                checked={allowedStaffNames.includes(name)}
...
function FileFormModal({ eventId, roles, staffNames, file, onClose, onSaved }) {
  ...
  const toggleStaff = (name) =>
    setAllowedStaffNames((prev) => prev.includes(name) ? prev.filter((n) => n !== name) : [...prev, name]);
  ...
  return (
    ...
    >
      <div className="bg-card border border-border rounded-t-2xl sm:rounded-2xl shadow-2xl w-full max-w-md p-4 space-y-3 max-h-[90vh] overflow-y-auto">
        ...
        <div className="space-y-2">
          ...
          <Input value={description} onChange={(e) => setDescription(e.target.value)} placeholder="説明・備考（任意）" className="h-8 text-sm" />
        </div>
              ...
              {staffNames.map((name) => (
                <button
                  ...
                  className={`px-2 py-0.5 rounded-full text-[11px] border font-medium transition-colors ${
                    allowedStaffNames.includes(name)
...
export default function SharedFileManager({ eventId, showAll = false }) {
  ...
  const { data: staffList = [] } = useQuery({
    queryKey: ["staff-for-files", eventId],
    queryFn: () => base44.entities.Staff.filter({ event_id: eventId }, "name"),
  });
  const staffNames = [...new Set(staffList.map((s) => s.name).filter(Boolean))].sort();
                ...
                <div className="flex-1 min-w-0">
                  ...
                  <div className="flex items-center gap-2 mt-1 flex-wrap">
                    <div className="relative">
                      <button
                        onClick={() => setOpenPanelId(isPanelOpen ? null : f.id)}
                        className={`text-[10px] px-1.5 py-0.5 rounded border font-medium cursor-pointer hover:opacity-80 transition-opacity ${badge.className}`}
                      >
                        {badge.label}
                      </button>
                      {isPanelOpen && (
